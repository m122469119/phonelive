...
//    public void getBitmapFromNet(SubsamplingScaleImageView imageView, String url) {
//        // AsyncTask 异步封装的工具, 可以实现异步请求及主界面更新(对线程池+handler的封装)
//        Bitmap bitmap = null;
//        if ((bitmap=mMemoryCacheUtils.getMemoryCache(url))!=null){
...
//            imageView.setImage(ImageSource.bitmap(bitmap));
//        } else {
//            new BitmapTask(true).execute(imageView, url);// 启动AsyncTask
//            Log.d("image", "这是从网络中加载的图片");
//        }
//    }
    /**
...
//             publishProgress(values);// 调用此方法实现进度更新(会回调onProgressUpdate)
                Log.d("imageDetailList", url);
                return bitmap;
            } else {
                bigImage = (ImageView) params[0];
                url = (String) params[1];
...
//                if (url.equals(this.url)) {// 判断图片绑定的url是否就是当前bitmap的url,
                    // 如果是,说明图片正确
                    bigImage.setImageBitmap(result);
                    System.out.println("从网络加载图片啦!!!");
